...
def run_random_split_experiment(
    ...
    """Run random-split evaluation and save only numerical/tabular outputs."""
...
def run_donor_cv_experiment(
    ...
    """Run donor-held-out CV and save only numerical/tabular outputs."""
    results_dir = Path(results_dir)
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    for rep_label, rep_key in representations.items():
        ...
        fold_metrics.to_csv(
            results_dir / f"{scheme_label}_{rep_label}_fold_metrics.csv", index=False
        )
        pd.concat(all_pred_rows, ignore_index=True).to_csv(
            results_dir / f"{scheme_label}_{rep_label}_all_predictions.csv", index=False
        )
        per_class_all = pd.concat(per_class_tables, ignore_index=True)
        per_class_all.to_csv(
            results_dir / f"{scheme_label}_{rep_label}_all_per_class_f1.csv", index=False
        )
        ...
        if label_col not in per_class_all.columns:
            if "cell_type" in per_class_all.columns:
                ...
                raise KeyError(
                    f"Could not find cell-type label column. Available columns: "
                    f"{per_class_all.columns.tolist()}"
                )

        per_class_mean = (
            per_class_all.groupby(label_col, as_index=False)[
