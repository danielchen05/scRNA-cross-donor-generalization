...
def _legacy_downsample(adata, group_cols: list[str], max_cells: int, random_state: int):
    """Downsample with NumPy RandomState to match legacy np.random.seed/choice behavior."""
...
def run_preprocessing(config: PreprocessingConfig, *, force: bool = False):
    ...
    # Freeze the supported label set before donor-level downsampling.
    #
    # The support rule defines which cell types enter the benchmark.
    # Downsampling can later reduce a retained cell type below min_cells
    # without making that label invalid.
    ...
    # Downsampling must not change the frozen set of benchmark cell types.
    # Check this before PCA/Harmony/scVI so failures occur before model training.
    downsampled_celltypes = set(
        adata.obs[config.celltype_col].astype(str).unique()
    )

    unexpected_celltypes = sorted(
        downsampled_celltypes - supported_celltypes
    )
    ...
    lost_celltypes = sorted(
        supported_celltypes - downsampled_celltypes
    )
    if lost_celltypes:
        raise ValueError(
    ...
    # A retained label may legitimately have fewer than min_cells after
    # donor-level subsampling. Instead, verify that the final object contains
    # exactly the label set frozen by the support-filter stage.
    final_celltypes = set(
        adata.obs[config.celltype_col].astype(str).unique()
    )

    unexpected_celltypes = sorted(
        final_celltypes - supported_celltypes
    )
    if unexpected_celltypes:
        raise ValueError(
    ...
    lost_celltypes = sorted(
        supported_celltypes - final_celltypes
    )
    if lost_celltypes:
        raise ValueError(
